...
export const Cell = ({ type, value, component: Component }) => {
    ...
    const cellAssert = (type, value) => {
        switch (type) {
            ...
            case 'button':
                return _jsx(_Fragment, { children: _jsx(Button, { size: "tiny", text: value.text, onClick: () => handleClick() }) });
            case 'link':
                if (!!Component)
            ...
            case 'image':
                return (_jsx("div", { className: "flex h-[64px] px-6 py-4", style: { backgroundImage: `linear-gradient(to left, rgba(44, 51, 53, 1), rgba(44, 51, 53, 0.5), rgba(44, 51, 53, 0.5), rgba(44, 51, 53, 0.2)), url('${value.imageUrl}')`, backgroundSize: 'cover' }, children: value.text }));
            default:
                return _jsx(_Fragment, {});
        }
    };
